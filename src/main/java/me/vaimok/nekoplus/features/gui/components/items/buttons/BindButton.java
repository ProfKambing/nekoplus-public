...
public class BindButton extends Button {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (isListening) {
            nekoplus.textManager.drawStringWithShadow("Listening...", x + 2.3F, y - 1.7F - me.vaimok.nekoplus.features.gui.nekoplusGui.getClickGui().getTextOffset(), getState() ? 0xFFFFFFFF : 0xFFAAAAAA);
        } else {
            nekoplus.textManager.drawStringWithShadow(setting.getName() + " " + TextUtil.GRAY + ((Bind) setting.getValue()).toString(), x + 2.3F, y - 1.7F - me.vaimok.nekoplus.features.gui.nekoplusGui.getClickGui().getTextOffset(), getState() ? 0xFFFFFFFF : 0xFFAAAAAA);
        }
    }
