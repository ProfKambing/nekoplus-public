...
class WeebAura extends Module {
    ...
    public
    WeebAura ( ) {
        super ( "WeebAura" , "Traps other players in webs" , Module.Category.COMBAT , true , false , false );
    }

    @Override
    public
    void onEnable ( ) {
        if ( me.vaimok.nekoplus.features.modules.combat.WeebAura.fullNullCheck ( ) ) {
            return;
        }
        startPos = EntityUtil.getRoundedBlockPos ( me.vaimok.nekoplus.features.modules.combat.WeebAura.mc.player );
        lastHotbarSlot = me.vaimok.nekoplus.features.modules.combat.WeebAura.mc.player.inventory.currentItem;
    ...
    void placeList ( List < Vec3d > list ) {
        list.sort ( ( vec3d , vec3d2 ) -> Double.compare ( me.vaimok.nekoplus.features.modules.combat.WeebAura.mc.player.getDistanceSq ( vec3d2.x , vec3d2.y , vec3d2.z ) , me.vaimok.nekoplus.features.modules.combat.WeebAura.mc.player.getDistanceSq ( vec3d.x , vec3d.y , vec3d.z ) ) );
    ...
    boolean check ( ) {
        ...
        if ( disable.getValue ( ).booleanValue ( ) && ! startPos.equals ( EntityUtil.getRoundedBlockPos ( me.vaimok.nekoplus.features.modules.combat.WeebAura.mc.player ) ) ) {
            disable ( );
            return true;
        }
        if ( obbySlot == - 1 ) {
        ...
        if ( me.vaimok.nekoplus.features.modules.combat.WeebAura.mc.player.inventory.currentItem != lastHotbarSlot && me.vaimok.nekoplus.features.modules.combat.WeebAura.mc.player.inventory.currentItem != obbySlot ) {
            lastHotbarSlot = me.vaimok.nekoplus.features.modules.combat.WeebAura.mc.player.inventory.currentItem;
        }
        switchItem ( true );
        isSneaking = EntityUtil.stopSneaking ( isSneaking );
    ...
    EntityPlayer getTarget ( double range ) {
        ...
        for (EntityPlayer player : me.vaimok.nekoplus.features.modules.combat.WeebAura.mc.world.playerEntities) {
            if ( EntityUtil.isntValid ( player , range ) || player.isInWeb || nekoplus.speedManager.getPlayerSpeed ( player ) > 30.0 )
                continue;
            if ( target == null ) {
                target = player;
                distance = me.vaimok.nekoplus.features.modules.combat.WeebAura.mc.player.getDistanceSq ( player );
                continue;
            }
            if ( ! ( me.vaimok.nekoplus.features.modules.combat.WeebAura.mc.player.getDistanceSq ( player ) < distance ) )
                continue;
            target = player;
            distance = me.vaimok.nekoplus.features.modules.combat.WeebAura.mc.player.getDistanceSq ( player );
        }
    ...
    void placeBlock ( BlockPos pos ) {
        if ( placements < blocksPerPlace.getValue ( ) && me.vaimok.nekoplus.features.modules.combat.WeebAura.mc.player.getDistanceSq ( pos ) <= MathUtil.square ( 6.0 ) && switchItem ( false ) ) {
            isPlacing = true;
