...
import me.vaimok.nekoplus.features.modules.misc.NoSoundLag;
import me.vaimok.nekoplus.features.setting.Bind;
import me.vaimok.nekoplus.features.setting.Setting;
import me.vaimok.nekoplus.util.Timer;
import me.vaimok.nekoplus.util.*;
...
class NekoAura
    ...
    private static me.vaimok.nekoplus.features.modules.combat.NekoAura instance;
    ...
    public Setting < AntiFriendPop > antiFriendPop = this.register ( new Setting < Object > ( "AntiFriendPop" , AntiFriendPop.NONE , v -> this.setting.getValue ( ) == Settings.MISC ) );
    ...
    public NekoAura( ) {
        super ( "NekoAura" , "Best CA on the market (Optimized)." , Module.Category.COMBAT , true , false , false );
        instance = this;
    }

    public static me.vaimok.nekoplus.features.modules.combat.NekoAura getInstance ( ) {
        if ( instance == null ) {
            instance = new me.vaimok.nekoplus.features.modules.combat.NekoAura( );
        }
        return instance;
    }
    ...
    void onDisable ( ) {
        ...
        Command.sendMessage(TextUtil.LIGHT_PURPLE + "No more sex :,(");
    }
    ...
    void onEnable ( ) {
        ...
        Command.sendMessage(TextUtil.LIGHT_PURPLE + "We sex :flushed:");
    }
    ...
    void onPacketSend ( PacketEvent.Send event ) {
        ...
        BlockPos pos = null;
        if ( event.getStage ( ) == 0 && event.getPacket ( ) instanceof CPacketUseEntity && ( packet = event.getPacket ( ) ).getAction ( ) == CPacketUseEntity.Action.ATTACK && packet.getEntityFromWorld ( me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.world ) instanceof EntityEnderCrystal ) {
            pos = Objects.requireNonNull ( packet.getEntityFromWorld ( me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.world ) ).getPosition ( );
            if ( this.removeAfterAttack.getValue ( ) ) {
                Objects.requireNonNull ( packet.getEntityFromWorld ( me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.world ) ).setDead ( );
                me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.world.removeEntityFromWorld ( packet.entityId );
            }
        }

        if ( event.getStage ( ) == 0 && event.getPacket ( ) instanceof CPacketUseEntity && ( packet = event.getPacket ( ) ).getAction ( ) == CPacketUseEntity.Action.ATTACK && packet.getEntityFromWorld ( me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.world ) instanceof EntityEnderCrystal ) {
            EntityEnderCrystal crystal = (EntityEnderCrystal) packet.getEntityFromWorld ( me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.world );
    ...
    void onPacketReceive ( PacketEvent.Receive event ) {
        ...
        if ( me.vaimok.nekoplus.features.modules.combat.NekoAura.fullNullCheck ( ) ) {
            return;
        }
        if ( ! this.justRender.getValue ( ) && this.switchTimer.passedMs ( this.switchCooldown.getValue ( ) ) && this.explode.getValue ( ) && this.instant.getValue ( ) && event.getPacket ( ) instanceof SPacketSpawnObject && ( this.syncedCrystalPos == null || ! this.syncedFeetPlace.getValue ( ) || this.damageSync.getValue ( ) == DamageSync.NONE ) ) {
            ...
            if ( packet2.getType ( ) == 51 && me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.player.getDistanceSq ( pos = new BlockPos ( packet2.getX ( ) , packet2.getY ( ) , packet2.getZ ( ) ) ) + (double) this.predictOffset.getValue ( ) <= MathUtil.square ( this.breakRange.getValue ( ) ) && ( this.instantTimer.getValue ( ) == PredictTimer.NONE || this.instantTimer.getValue ( ) == PredictTimer.BREAK && this.breakTimer.passedMs ( this.breakDelay.getValue ( ) ) || this.instantTimer.getValue ( ) == PredictTimer.PREDICT && this.predictTimer.passedMs ( this.predictDelay.getValue ( ) ) ) ) {
                ...
                if ( this.predictFriendDmg.getValue ( ) && ( this.antiFriendPop.getValue ( ) == AntiFriendPop.BREAK || this.antiFriendPop.getValue ( ) == AntiFriendPop.ALL ) && this.isRightThread ( ) ) {
                    for (EntityPlayer friend : me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.world.playerEntities) {
                        if ( friend == null || me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.player.equals ( friend ) || friend.getDistanceSq ( pos ) > MathUtil.square ( this.range.getValue ( ) + this.placeRange.getValue ( ) ) || ! nekoplus.friendManager.isFriend ( friend ) || ! ( (double) DamageUtil.calculateDamage ( pos , friend ) > (double) EntityUtil.getHealth ( friend ) + 0.5 ) )
                            continue;
                        return;
                    }
                }
                if ( placedPos.contains ( pos.down ( ) ) ) {
                    float selfDamage;
                    if ( this.isRightThread ( ) && this.superSafe.getValue ( ) ? DamageUtil.canTakeDamage ( this.suicide.getValue ( ) ) && ( (double) ( selfDamage = DamageUtil.calculateDamage ( pos , me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.player ) ) - 0.5 > (double) EntityUtil.getHealth ( me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.player ) || selfDamage > this.maxSelfBreak.getValue ( ) ) : this.superSafe.getValue ( ) ) {
                        return;
                    }
                    ...
                    if ( DamageUtil.canTakeDamage ( this.suicide.getValue ( ) ) ) {
                        selfDamage = DamageUtil.calculateDamage ( pos , me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.player );
                    }
                    if ( (double) selfDamage + 0.5 < (double) EntityUtil.getHealth ( me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.player ) && selfDamage <= this.maxSelfBreak.getValue ( ) ) {
                        for (EntityPlayer player : me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.world.playerEntities) {
                            float damage;
            ...
            for (int id : packet4.getEntityIDs ( )) {
                Entity entity = me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.world.getEntityByID ( id );
            ...
            if ( packet5.getOpCode ( ) == 35 && packet5.getEntity ( me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.world ) instanceof EntityPlayer ) {
                this.totemPops.put ( (EntityPlayer) packet5.getEntity ( me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.world ) , new Timer ( ).reset ( ) );
            }
            ...
            if ( this.threadMode.getValue ( ) == ThreadMode.SOUND && this.isRightThread ( ) && me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.player != null && me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.player.getDistanceSq ( pos ) < MathUtil.square ( this.soundPlayer.getValue ( ) ) ) {
    ...
    boolean isRightThread ( ) {
        return mc.isCallingFromMinecraftThread ( ) || ! nekoplus.eventManager.ticksOngoing ( ) && ! this.threadOngoing.get ( );
    }
    ...
    void attackCrystalPredict ( int entityID , BlockPos pos ) {
        ...
        me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.player.connection.sendPacket ( attackPacket );
        if ( this.breakSwing.getValue ( ) ) {
            me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.player.connection.sendPacket ( new CPacketAnimation ( EnumHand.MAIN_HAND ) );
        }
    ...
    void postProcessBreak ( ) {
        while ( ! this.packetUseEntities.isEmpty ( ) ) {
            ...
            me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.player.connection.sendPacket ( packet );
            if ( this.breakSwing.getValue ( ) ) {
                me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.player.swingArm ( EnumHand.MAIN_HAND );
            }
            this.breakTimer.reset ( );
        }
    }
    ...
    boolean check ( ) {
        ...
        this.mainHand = me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.player.getHeldItemMainhand ( ).getItem ( ) == Items.END_CRYSTAL;
        this.offHand = me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.player.getHeldItemOffhand ( ).getItem ( ) == Items.END_CRYSTAL;
        this.currentDamage = 0.0;
        this.placePos = null;
        if ( this.lastSlot != me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.player.inventory.currentItem || AutoTrap.isPlacing || Surround.isPlacing ) {
            this.lastSlot = me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.player.inventory.currentItem;
        ...
        if ( ! ( ( this.offHand || this.mainHand || this.switchMode.getValue ( ) != Switch.BREAKSLOT || this.switching ) && DamageUtil.canBreakWeakness ( me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.player ) && this.switchTimer.passedMs ( this.switchCooldown.getValue ( ) ) ) ) {
            this.renderPos = null;
            target = null;
            this.rotating = false;
            return false;
        }
        if ( this.mineSwitch.getValue ( ) && Mouse.isButtonDown ( 0 ) && ( this.switching || this.autoSwitch.getValue ( ) == AutoSwitch.ALWAYS ) && Mouse.isButtonDown ( 1 ) && me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.player.getHeldItemMainhand ( ).getItem ( ) instanceof ItemPickaxe ) {
            this.switchItem ( );
        }
        this.mapCrystals ( );
    ...
    void mapCrystals ( ) {
        ...
        for (Entity entity : me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.world.loadedEntityList) {
            ...
            float selfDamage = - 1.0f;
            if ( DamageUtil.canTakeDamage ( this.suicide.getValue ( ) ) ) {
                selfDamage = DamageUtil.calculateDamage ( entity , me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.player );
            }
            if ( (double) selfDamage + 0.5 < (double) EntityUtil.getHealth ( me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.player ) && selfDamage <= this.maxSelfBreak.getValue ( ) ) {
                ...
                for (EntityPlayer player : me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.world.playerEntities) {
                    ...
                    if ( this.antiFriendPop.getValue ( ) != AntiFriendPop.BREAK && this.antiFriendPop.getValue ( ) != AntiFriendPop.ALL || ! nekoplus.friendManager.isFriend ( player.getName ( ) ) || ! ( (double) ( damage = DamageUtil.calculateDamage ( entity , player ) ) > (double) EntityUtil.getHealth ( player ) + 0.5 ) )
                        continue;
        ...
        if ( this.webAttack.getValue ( ) && this.webPos != null ) {
            if ( me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.player.getDistanceSq ( this.webPos.up ( ) ) > MathUtil.square ( this.breakRange.getValue ( ) ) ) {
                this.webPos = null;
            } else {
                for (Entity entity : me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.world.getEntitiesWithinAABB ( Entity.class , new AxisAlignedBB ( this.webPos.up ( ) ) )) {
                    if ( ! ( entity instanceof EntityEnderCrystal ) ) continue;
    ...
    private
    boolean doSwitch ( ) {
        if ( me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.player.getHeldItemOffhand ( ).getItem ( ) == Items.END_CRYSTAL ) {
            this.mainHand = false;
        } else {
    ...
    void calculateDamage ( EntityPlayer targettedPlayer ) {
        ...
        if ( this.webAttack.getValue ( ) && targettedPlayer != null && ( web = me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.world.getBlockState ( playerPos = new BlockPos ( targettedPlayer.getPositionVector ( ) ) ).getBlock ( ) ) == Blocks.WEB ) {
            setToAir = playerPos;
            state = me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.world.getBlockState ( playerPos );
            me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.world.setBlockToAir ( playerPos );
        }
        block0:
        for (BlockPos pos : BlockUtil.possiblePlacePositions ( this.placeRange.getValue ( ) , this.antiSurround.getValue ( ) , this.oneDot15.getValue ( ) )) {
            if ( ! BlockUtil.rayTracePlaceCheck ( pos , ( this.raytrace.getValue ( ) == Raytrace.PLACE || this.raytrace.getValue ( ) == Raytrace.FULL ) && me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.player.getDistanceSq ( pos ) > MathUtil.square ( this.placetrace.getValue ( ) ) , 1.0f ) )
                continue;
            float selfDamage = - 1.0f;
            if ( DamageUtil.canTakeDamage ( this.suicide.getValue ( ) ) ) {
                selfDamage = DamageUtil.calculateDamage ( pos , me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.player );
            }
            if ( ! ( (double) selfDamage + 0.5 < (double) EntityUtil.getHealth ( me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.player ) ) || ! ( selfDamage <= this.maxSelfPlace.getValue ( ) ) )
                continue;
            if ( targettedPlayer != null ) {
                ...
                if ( this.calcEvenIfNoDamage.getValue ( ) && ( this.antiFriendPop.getValue ( ) == AntiFriendPop.ALL || this.antiFriendPop.getValue ( ) == AntiFriendPop.PLACE ) ) {
                    ...
                    for (EntityPlayer friend : me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.world.playerEntities) {
                        float friendDamage;
                        if ( friend == null || me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.player.equals ( friend ) || friend.getDistanceSq ( pos ) > MathUtil.square ( this.range.getValue ( ) + this.placeRange.getValue ( ) ) || ! nekoplus.friendManager.isFriend ( friend ) || ! ( (double) ( friendDamage = DamageUtil.calculateDamage ( pos , friend ) ) > (double) EntityUtil.getHealth ( friend ) + 0.5 ) )
                            continue;
                        friendPop = true;
                        break;
                    }
        ...
        if ( setToAir != null ) {
            me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.world.setBlockState ( setToAir , state );
    ...
    EntityPlayer getTarget ( boolean unsafe ) {
        ...
        for (EntityPlayer player : me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.world.playerEntities) {
            ...
            if ( ! ( me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.player.getDistanceSq ( player ) < me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.player.getDistanceSq ( currentTarget ) ) )
                continue;
            currentTarget = player;
        }
        ...
        if ( this.predictPos.getValue ( ) && currentTarget != null ) {
            ...
            EntityOtherPlayerMP newTarget = new EntityOtherPlayerMP ( me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.world , profile );
    ...
    void doFakeSwing ( ) {
        if ( this.fakeSwing.getValue ( ) ) {
            EntityUtil.swingArmNoPacket ( EnumHand.MAIN_HAND , me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.player );
        }
    }
    ...
    void manualBreaker ( ) {
        ...
        if ( this.rotate.getValue ( ) != Rotate.OFF && this.eventMode.getValue ( ) != 2 && this.rotating ) {
            if ( this.didRotation ) {
                me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.player.rotationPitch = (float) ( (double) me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.player.rotationPitch + 4.0E-4 );
        ...
        if ( ( this.offHand || this.mainHand ) && this.manual.getValue ( ) && this.manualTimer.passedMs ( this.manualBreak.getValue ( ) ) && Mouse.isButtonDown ( 1 ) && me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.player.getHeldItemOffhand ( ).getItem ( ) != Items.GOLDEN_APPLE && me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.player.inventory.getCurrentItem ( ).getItem ( ) != Items.GOLDEN_APPLE && me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.player.inventory.getCurrentItem ( ).getItem ( ) != Items.BOW && me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.player.inventory.getCurrentItem ( ).getItem ( ) != Items.EXPERIENCE_BOTTLE && ( result = me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.objectMouseOver ) != null ) {
            switch (result.typeOfHit) {
                ...
                case BLOCK: {
                    BlockPos mousePos = me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.objectMouseOver.getBlockPos ( ).up ( );
                    for (Entity target : me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.world.getEntitiesWithinAABBExcludingEntity ( null , new AxisAlignedBB ( mousePos ) )) {
    ...
    void rotateTo ( Entity entity ) {
        switch (this.rotate.getValue ( )) {
            ...
            case BREAK:
            case ALL: {
                float[] angle = MathUtil.calcAngle ( me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.player.getPositionEyes ( mc.getRenderPartialTicks ( ) ) , entity.getPositionVector ( ) );
    ...
    void rotateToPos ( BlockPos pos ) {
        switch (this.rotate.getValue ( )) {
            ...
            case PLACE:
            case ALL: {
                float[] angle = MathUtil.calcAngle ( me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.player.getPositionEyes ( mc.getRenderPartialTicks ( ) ) , new Vec3d ( (float) pos.getX ( ) + 0.5f , (float) pos.getY ( ) - 0.5f , (float) pos.getZ ( ) + 0.5f ) );
                if ( this.eventMode.getValue ( ) == 2 && this.threadMode.getValue ( ) == ThreadMode.NONE ) {
                    nekoplus.rotationManager.setPlayerRotations ( angle[0] , angle[1] );
                    break;
                }
    ...
    boolean isValid ( Entity entity ) {
        return entity != null && me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.player.getDistanceSq ( entity ) <= MathUtil.square ( this.breakRange.getValue ( ) ) && ( this.raytrace.getValue ( ) == Raytrace.NONE || this.raytrace.getValue ( ) == Raytrace.PLACE || me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.player.canEntityBeSeen ( entity ) || ! me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.player.canEntityBeSeen ( entity ) && me.vaimok.nekoplus.features.modules.combat.NekoAura.mc.player.getDistanceSq ( entity ) <= MathUtil.square ( this.breaktrace.getValue ( ) ) );
    }
    ...
    class PlaceInfo {
        ...
        public
        PlaceInfo ( BlockPos pos , boolean offhand , boolean placeSwing , boolean exactHand ) {
            this.pos = pos;
    ...
    class RAutoCrystal
        ...
        private me.vaimok.nekoplus.features.modules.combat.NekoAura nekoAura;
        ...
        public static
        RAutoCrystal getInstance ( me.vaimok.nekoplus.features.modules.combat.NekoAura nekoAura) {
            if ( instance == null ) {
                instance = new RAutoCrystal ( );
                RAutoCrystal.instance.nekoAura = nekoAura;
            }
            return instance;
        }
        ...
        public
        void run ( ) {
            if ( this.nekoAura.threadMode.getValue ( ) == ThreadMode.WHILE ) {
                while ( this.nekoAura.isOn ( ) && this.nekoAura.threadMode.getValue ( ) == ThreadMode.WHILE ) {
                    while ( nekoplus.eventManager.ticksOngoing ( ) ) {
                    }
                    if ( this.nekoAura.shouldInterrupt.get ( ) ) {
                        this.nekoAura.shouldInterrupt.set ( false );
                        this.nekoAura.syncroTimer.reset ( );
                        this.nekoAura.thread.interrupt ( );
                        break;
                    }
                    this.nekoAura.threadOngoing.set ( true );
                    nekoplus.safetyManager.doSafetyCheck ( );
                    this.nekoAura.doAutoCrystal ( );
                    this.nekoAura.threadOngoing.set ( false );
                    try {
                        Thread.sleep ( this.nekoAura.threadDelay.getValue ( ) );
                    } catch ( InterruptedException e ) {
                        this.nekoAura.thread.interrupt ( );
                        e.printStackTrace ( );
                    }
                }
            } else if ( this.nekoAura.threadMode.getValue ( ) != ThreadMode.NONE && this.nekoAura.isOn ( ) ) {
                while ( nekoplus.eventManager.ticksOngoing ( ) ) {
                }
                this.nekoAura.threadOngoing.set ( true );
                nekoplus.safetyManager.doSafetyCheck ( );
