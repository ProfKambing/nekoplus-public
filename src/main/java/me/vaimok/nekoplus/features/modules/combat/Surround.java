...
import me.vaimok.nekoplus.util.InventoryUtil;
...
class Surround extends Module {
    ...
    private final Setting < Integer > delay = register ( new Setting ( "Delay" , 0 , 0 , 250 ) );
    private final Setting < Integer > blocksPerTick = this.register ( new Setting < Integer > ( "BlocksPerTick" , 12 , 1 , 20 ) );
    private final me.vaimok.nekoplus.util.Timer timer = new me.vaimok.nekoplus.util.Timer ( );
    private final me.vaimok.nekoplus.util.Timer retryTimer = new Timer ( );
    ...
    void onEnable ( ) {
        ...
        if ( this.center.getValue ( ).booleanValue ( ) ) {
            nekoplus.positionManager.setPositionPacket ( (double) this.startPos.getX ( ) + 0.5 , this.startPos.getY ( ) , (double) this.startPos.getZ ( ) + 0.5 , true , true , true );
        }
        this.retries.clear ( );
        ...
        for (Vec3d vec3d : vec3ds) {
            ...
            switch (BlockUtil.isPositionPlaceable ( position , false )) {
                case 1: {
                    ...
                    if ( nekoplus.speedManager.getSpeedKpH ( ) != 0.0 || isExtending || this.extenders >= 1 )
                        continue block5;
