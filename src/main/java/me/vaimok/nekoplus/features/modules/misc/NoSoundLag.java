...
class NoSoundLag
    ...
    void onPacketReceived ( PacketEvent.Receive event ) {
        if ( event != null && event.getPacket ( ) != null && NoSoundLag.mc.player != null && NoSoundLag.mc.world != null && event.getPacket ( ) instanceof SPacketSoundEffect ) {
            ...
            if ( this.crystals.getValue ( ).booleanValue ( ) && packet.getCategory ( ) == SoundCategory.BLOCKS && packet.getSound ( ) == SoundEvents.ENTITY_GENERIC_EXPLODE && ( NekoAura.getInstance ( ).isOff ( ) || ! NekoAura.getInstance ( ).sound.getValue ( ).booleanValue ( ) && NekoAura.getInstance ( ).threadMode.getValue ( ) != NekoAura.ThreadMode.SOUND ) ) {
