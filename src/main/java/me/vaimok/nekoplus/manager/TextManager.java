...
import me.vaimok.nekoplus.features.gui.font.CustomFont;
import me.vaimok.nekoplus.features.modules.client.FontMod;
...
public
class TextManager extends Feature {
    ...
    void init ( boolean startup ) {
        FontMod cFont = nekoplus.moduleManager.getModuleByClass ( FontMod.class );
        try {
    ...
    void drawString ( String text , float x , float y , int color , boolean shadow ) {
        if ( nekoplus.moduleManager.isModuleEnabled ( FontMod.class ) ) {
            if ( shadow ) {
