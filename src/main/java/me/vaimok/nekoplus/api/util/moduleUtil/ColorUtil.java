...
class ColorUtil {
    ...
    public static
    int toRGBA ( double r , double g , double b , double a ) {
        return toRGBA ( (float) r , (float) g , (float) b , (float) a );
    }

    public static
    int toRGBA ( int r , int g , int b ) {
    ...
    public static
    int toARGB ( int r , int g , int b , int a ) {
        return new Color ( r , g , b , a ).getRGB ( );
        //return (a << 24) + (r << 16) + (g << 8) + (b);
    }

    public static
    int toRGBA ( float r , float g , float b , float a ) {
        return toRGBA ( (int) ( r * 255.f ) , (int) ( g * 255.f ) , (int) ( b * 255.f ) , (int) ( a * 255.f ) );
    }

    public static
    int toRGBA ( float[] colors ) {
    ...
    public static
    Color rainbow ( int delay ) {
        double rainbowState = Math.ceil ( (double) ( System.currentTimeMillis ( ) + (long) delay ) / 20.0 );
        return Color.getHSBColor ( (float) ( ( rainbowState %= 360.0 ) / 360.0 ) , 200 / 255.0f , 255 / 255.0f );
    }

    private
    ArrayList < ColorName > initColorList ( ) {
    ...
    public
    String getColorNameFromHex ( int hexColor ) {
