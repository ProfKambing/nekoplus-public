...
class InventoryUtil implements Util {
    ...
    public static
    boolean isNull ( ItemStack stack ) {
    ...
    public static
    int findHotbarBlock ( Block blockIn ) {
        for (int i = 0; i < 9; i++) {
            ItemStack stack = mc.player.inventory.getStackInSlot ( i );
            if ( stack == ItemStack.EMPTY ) {
                continue;
    ...
    public static
    List < Integer > findEmptySlots ( boolean withXCarry ) {
        ...
        switch (mode) {
            ...
            case SILENT:
                if ( ! back && ! switchedItem ) {
                    ...
                    switchedItemSwitched[0] = false;
                    nekoplus.inventoryManager.recoverSilent ( lastHotbarSlot );
                }
                switchedItemSwitched[1] = true;
                break;
            case NONE:
    ...
    public static
    int convertHotbarToInv ( int input ) {
        return 45 - 9 + input;
    }
    ...
    public static
    EntityEquipmentSlot getEquipmentFromSlot ( int slot ) {
        if ( slot == 5 ) {
