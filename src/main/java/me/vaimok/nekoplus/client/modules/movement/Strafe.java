...
class Strafe extends Module {
    ...
    void onMove ( MoveEvent event ) {
        ...
        if ( this.mode.getValue ( ) == Mode.NCP ) {
            ...
            if ( this.limiter2.getValue ( ).booleanValue ( ) && Strafe.mc.player.onGround && nekoplus.speedManager.getSpeedKpH ( ) < (double) this.speedLimit2.getValue ( ).floatValue ( ) ) {
                this.stage = 2;
            }
            ...
            if ( this.stage == 1 && EntityUtil.isMoving ( ) ) {
                ...
                if ( Strafe.mc.world.getCollisionBoxes ( Strafe.mc.player , Strafe.mc.player.getEntityBoundingBox ( ).offset ( 0.0 , Strafe.mc.player.motionY , 0.0 ) ).size ( ) > 0 || Strafe.mc.player.collidedVertically && this.stage > 0 ) {
                    this.stage = this.bhop2.getValue ( ) != false && nekoplus.speedManager.getSpeedKpH ( ) >= (double) this.speedLimit.getValue ( ).floatValue ( ) ? 0 : ( Strafe.mc.player.moveForward != 0.0f || Strafe.mc.player.moveStrafing != 0.0f ? 1 : 0 );
                }
    ...
    boolean shouldReturn ( ) {
        return nekoplus.moduleManager.isModuleEnabled ( Freecam.class );
    }
