...
import me.vaimok.nekoplus.api.event.events.DeathEvent;
...
import me.vaimok.nekoplus.client.modules.combat.NekoAura;
import me.vaimok.nekoplus.client.setting.Setting;
import me.vaimok.nekoplus.api.manager.FileManager;
...
class AutoEZ extends Module {
    private static final String path = "nekoplus/autoez.txt";
    ...
    void onTick ( ) {
        ...
        if ( NekoAura.target != null && this.cauraTarget != NekoAura.target ) {
            this.cauraTarget = NekoAura.target;
        }
        ...
        if ( NekoAura.target != null ) {
            this.targets.put ( NekoAura.target , (int) ( this.timer.getPassedTimeMs ( ) / 1000L ) );
        }
    ...
    void onAttackEntity ( AttackEntityEvent event ) {
        if ( event.getTarget ( ) instanceof EntityPlayer && ! nekoplus.friendManager.isFriend ( event.getEntityPlayer ( ) ) ) {
    ...
    void onSendAttackPacket ( PacketEvent.Send event ) {
        ...
        if ( event.getPacket ( ) instanceof CPacketUseEntity && ( packet = event.getPacket ( ) ).getAction ( ) == CPacketUseEntity.Action.ATTACK && packet.getEntityFromWorld ( AutoEZ.mc.world ) instanceof EntityPlayer && ! nekoplus.friendManager.isFriend ( (EntityPlayer) packet.getEntityFromWorld ( AutoEZ.mc.world ) ) ) {
