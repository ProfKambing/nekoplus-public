...
import me.vaimok.nekoplus.features.modules.player.FakePlayer;
import me.vaimok.nekoplus.features.modules.player.Freecam;
...
class EntityUtil implements Util {
    ...
    boolean isCrystalAtFeet ( final EntityEnderCrystal crystal , final double range ) {
        for (final EntityPlayer player : mc.world.playerEntities) {
            ...
            if ( nekoplus.friendManager.isFriend ( player ) ) {
                continue;
            }
    ...
    public static
    float getDifficultyMultiplier ( float distance ) {
    ...
    public static
    int getEnchantmentModifierDamage ( Iterable < ItemStack > stacks , DamageSource source ) {
        int modifier = 0;
    ...
    public static
    float calculate ( double x , double y , double z , EntityLivingBase base ) {
   ...
   public static
    ...
    public static
    double[] calculateLookAt ( double px , double py , double pz , EntityPlayer me ) {
    ...
    public static
    boolean isMobAggressive ( Entity entity ) {
    ...
    public static
    void swingArmNoPacket ( EnumHand hand , EntityLivingBase entity ) {
    ...
    public static
    int getArmSwingAnimationEnd ( EntityLivingBase entity ) {
    ...
    public static
    boolean isDrivenByPlayer ( Entity entityIn ) {
    ...
    boolean isntValid ( Entity entity , double range ) {
        return entity == null || isDead ( entity ) || entity.equals ( mc.player ) || ( entity instanceof EntityPlayer && nekoplus.friendManager.isFriend ( entity.getName ( ) ) ) || mc.player.getDistanceSq ( entity ) > MathUtil.square ( range );
    }
    ...
    public static
    boolean holdingWeapon ( EntityPlayer player ) {
    ...
    public static
    boolean isEntityMoving ( final Entity entity ) {
        if ( entity == null ) {
            return false;
        }
    ...
    public static
    double getEntitySpeed ( final Entity entity ) {
        if ( entity != null ) {
    ...
    public static
    boolean holding32k ( EntityPlayer player ) {
    ...
    public static
    boolean simpleIs32k ( ItemStack stack ) {
    ...
    Color getColor ( Entity entity , int red , int green , int blue , int alpha , boolean colorFriends ) {
        ...
        if ( entity instanceof EntityPlayer ) {
            if ( colorFriends && nekoplus.friendManager.isFriend ( (EntityPlayer) entity ) ) {
                color = new Color ( 85 / 255.0f , 255 / 255.0f , 255 / 255.0f , alpha / 255.0f );
            }

            /*Killaura killaura = (Killaura) nekoplus.moduleManager.getModuleByClass(Killaura.class);
    ...
    public static
    boolean isOnLiquid ( double offset ) {
        if ( mc.player.fallDistance >= 3.0f ) {
            return false;
        }
    ...
    public static
    boolean isAboveLiquid ( Entity entity ) {
        if ( entity == null ) {
            return false;
        }
    ...
    public static
    boolean checkForLiquid ( Entity entity , boolean b ) {
        if ( entity == null ) {
            return false;
        }
    ...
    Map < String, Integer > getTextRadarPlayers ( ) {
        ...
        for (EntityPlayer player : mc.world.playerEntities) {
            ...
            output.put ( healthSB + " " + ( nekoplus.friendManager.isFriend ( player ) ? TextUtil.AQUA : TextUtil.RESET ) + player.getName ( ) + " " + distanceSB + " " + TextUtil.WHITE + nekoplus.totemPopManager.getTotemPopString ( player ) + nekoplus.potionManager.getTextRadarPotion ( player ) , (int) mc.player.getDistance ( player ) );
